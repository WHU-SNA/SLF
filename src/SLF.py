...
class SignedLatentFactorModel(object):
    def __init__(self, args):
        ...
        self.W_out_p = np.zeros((self.num_node, self.args.k1))
        # Positive inward SLF vectors.
        self.W_in_p = np.zeros((self.num_node, self.args.k1))
        # Negative outward SLF vectors.
        self.W_out_n = np.zeros((self.num_node, self.args.k2))
        # Negative inward SLF vectors.
        self.W_in_n = np.zeros((self.num_node, self.args.k2))
    ...
    def fit(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for u in nodes:
                pbar.update(1)
                out_p_g = np.zeros(self.args.k1)
                out_n_g = np.zeros(self.args.k2)
                in_p_g = np.zeros(self.args.k1)
                in_n_g = np.zeros(self.args.k2)

                succs = G.successors(u)
                for succ in succs:
                    e_p = fa(self.W_out_p[u] @ self.W_in_p[succ], self.args)
                    e_n = fa(self.W_out_n[u] @ self.W_in_n[succ], self.args)
                ...
                for i in range(self.args.n):
                    ...
                    while noise in succs:
                        noise = rd.choice(nodes)
                    e_p = fa(self.W_out_p[u] @ self.W_in_p[noise], self.args)
                    e_n = fa(self.W_out_n[u] @ self.W_in_n[noise], self.args)
                    out_p_g -= e_p * self.W_in_p[noise]
                    out_n_g -= e_n * self.W_in_n[noise]

                pres = G.predecessors(u)
                for pre in pres:
                    e_p = fa(self.W_out_p[pre] @ self.W_in_p[u], self.args)
                    e_n = fa(self.W_out_n[pre] @ self.W_in_n[u], self.args)
                    if G[pre][u]['weight'] == 1:
                ...
                for i in range(self.args.n):
                    ...
                    while noise in pres:
                        noise = rd.choice(nodes)
                    e_p = fa(self.W_out_p[noise] @ self.W_in_p[u], self.args)
                    e_n = fa(self.W_out_n[noise] @ self.W_in_n[u], self.args)
            ...
            W_out = np.zeros((self.num_node, self.args.k1 + self.args.k2))
            W_in = np.zeros((self.num_node, self.args.k1 + self.args.k2))
